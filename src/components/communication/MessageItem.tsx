...
export default function MessageItem({ message, isOwn, showAvatar = true, showName = true, members = [], groupId, onTaskClick, onDelete, onReply }: MessageItemProps) {
  ...
  const [memberRole, setMemberRole] = useState<'project_owner' | 'project_admin' | 'project_member'>('member');
    ...
    if (profileData) {
      ...
      if (groupId) {
        const { data: memberData } = await supabase
          ...
          .eq('group_id', groupId)
          .single();
        setMemberRole((memberData?.role as 'project_owner' | 'project_admin' | 'project_member') || 'member');
      }
